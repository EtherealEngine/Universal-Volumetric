...
export default class Player {
  ...
  fetchBuffers = (callback?: fetchBuffersCallback) => {
    ...
    for (let i = 0; i < this.bufferDuration; i++) {
      ...
      if (
        this.lastRequestedGeometryFrame - this.currentGeometryFrame < this.bufferDuration * geometryBufferSize &&
        this.lastRequestedGeometryFrame != this.fileHeader.GeometryFrameCount - 1
      ) {
        let currentRequestingFrame = this.lastRequestedGeometryFrame + 1
        const currentRequestEnd = Math.min(
          this.currentGeometryFrame + (i + 1) * geometryBufferSize,
          this.fileHeader.GeometryFrameCount - 1
        )
        ...
        for (; currentRequestingFrame <= this.lastRequestedGeometryFrame; currentRequestingFrame++) {
          ...
          const dracoURL = this.fileHeader.DRCURLPattern.replace(
            '#'.repeat(padWidth),
            this.pad(currentRequestingFrame, padWidth)
          )
      ...
      if (
        this.lastRequestedTextureSegment - this.currentTextureSegment < this.bufferDuration * textureBufferSize &&
        this.lastRequestedTextureSegment != this.fileHeader.TextureSegmentCount - 1
      ) {
        let currentRequestingTextureSegment = this.lastRequestedTextureSegment + 1
        const currentRequestEnd = Math.min(
          this.currentTextureSegment + (i + 1) * textureBufferSize,
          this.fileHeader.TextureSegmentCount - 1
        )
        ...
        for (; currentRequestingTextureSegment <= this.lastRequestedTextureSegment; currentRequestingTextureSegment++) {
          ...
          const textureURL = this.fileHeader.KTX2URLPattern.replace(
            '#'.repeat(padWidth),
            this.pad(currentRequestingTextureSegment, padWidth)
          )
